defmodule Typi.ChatChannelTest do
  ...
  @message_attrs  %{body: "the body", created_at: :os.system_time(:seconds)}
  ...
  test "server receives message, stores it, creates status entries and replies with `sending` status",
      %{socket: socket, users: [john, mike, sam, sara], chat: chat} do
    {:ok, _, socket} = subscribe_and_join(socket, "chats:#{chat.id}", %{})
    ref = push socket, "message", @message_attrs
    assert_reply ref, :ok, %{id: _, status: "sending"}

    message = get_message()
    chat_id = chat.id
    john_id = john.id
    assert %Message{body: "the body", chat_id: ^chat_id, status: "sending", user_id: ^john_id} = message
  ...
  test "after message is received by the server, it broadcasts to all recipients currently in chat",
      %{socket: socket, users: [john, _mike, _sam, _sara], chat: chat} do
    {:ok, _, socket} = subscribe_and_join(socket, "chats:#{chat.id}", %{})
    _ref = push socket, "message", @message_attrs
    john_id = john.id
    assert_broadcast "message", %{body: "the body", userId: ^john_id, status: "sending"}
  end

  test "after message is received by the server it broadcasts to those who are not in the chat but are online",
      %{socket: socket, users: [john, mike, sam, sara], chat: chat} do
    {:ok, _, socket} = subscribe_and_join(socket, "chats:#{chat.id}")
  ...
  test "When message is received by a recipient, recipient sends the status `received`," <>
      "and if all statuses are `received` or `read`, it changes the status of the message" <>
      "and pushes it to owner via broadcast or APN",
      %{socket: socket, users: [john, mike, sam, sara], chat: chat} do
  ...
  test "when message is read, recipient sends the status `read`, which is pushed at owner via broadcast or APN",
      %{socket: socket, users: [john, mike, sam, sara], chat: chat} do
    # connect to users:... channel in order to see the status being pushed via `message:status`
    {:ok, _, _user_socket} = subscribe_and_join(socket, "users:#{john.id}", %{})
